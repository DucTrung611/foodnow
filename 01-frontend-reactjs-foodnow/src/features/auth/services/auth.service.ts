...
export const authService = {
  ...
  // refresh_token lives in an httpOnly cookie; this rotates it and returns a
  // fresh access token *and* the user it now belongs to — the cookie is
  // browser-wide, not per-tab, so `user` is how a tab notices its session
  // was silently swapped by a login in a different tab (see client.ts).
  refresh: () => unwrap<{ accessToken: string; user: User }>(apiClient.post('/auth/refresh')),
