...
- `POST /orders/quote` — CUSTOMER. Same `CreateOrderDto` body and pricing rules as `POST /orders` (both call the private `priceOrder()`), but read-only — no `ordersRepository.createOrder()` write, no promotion-usage recording. Added so checkout can show delivery fee + total before the customer commits (UX-AUDIT-REPORT.md §1.3 — previously only the subtotal was visible pre-order).
